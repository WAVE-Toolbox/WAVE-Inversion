...
\begin{adjustbox}{width=1\textwidth,center=\textwidth}
	...
	\midrule                 
         ...
         gradientType & Use migration or tomographic kernel (0, 1, 2, 3) & int & \num{0} \\
...
Note that seismograms can be normalized for the calculation of the misfit and the adjoint sources by setting \verb+normalizeTraces+=1. This option is recommended for seismic field data. The parameter \verb+gradientType+ can be used to perform reflection waveform inversion \citep{xu2012inversion} or reverse time migration (RTM). One can use migration kernel alone ( \verb+gradientType+=1) or tomographic kernel alone (\verb+gradientType+=2) or these two kernels interactively in inversion iteration (\verb+gradientType+=3). If \verb+gradientType+=4, RTM will be implemented once in each workflow stage, which is related to the imaging condition controlled by \verb+misfitType+. If \verb+decomposeWavefieldType+=0, these kernels are computed using the Born approximation \citep{yao2017reflection}. If \verb+decomposeWavefieldType+$>$0, Poynting vector method is used for kernel computation \citep{tang2013tomographically}.
...
\begin{adjustbox}{width=1\textwidth,center=\textwidth}
	...
	\midrule
         ...
         minOffsetSrcEst          & Minimum offset in grid points to allow                              &  int   & 0 \\
...
\end{cases}
\end{equation}
\par Derivative in respect to model parameters $\mathbf{m}$ from Eq. \ref{eqn:AugmentFunctional1}:
\begin{equation}
