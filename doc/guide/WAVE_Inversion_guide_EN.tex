...
Note that seismograms can be normalized for the calculation of the misfit and the adjoint sources by setting \verb+normalizeTraces+=1. This option is recommended for seismic field data. The parameter \verb+gradientType+ can be used to perform reflection waveform inversion \citep{xu2012inversion}. One can use migration kernel alone ( \verb+gradientType+=1) or tomographic kernel alone (\verb+gradientType+=2) or these two kernels interactively in inversion iteration (\verb+gradientType+=3). If \verb+decomposeType+=0, these kernels are computed using the Born approximation \citep{yao2017reflection}. If \verb+decomposeType+$>$0, Poynting vector method is used for kernel computation \citep{tang2013tomographically}.
