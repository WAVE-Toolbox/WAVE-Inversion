...
\begin{adjustbox}{width=1\textwidth,center=\textwidth}
	...
	\midrule                 
         ...
         misfitTypeFilename               & Filename of misfit type                                                    & string & logs/misfitType.txt  \\
         ...
         a\_Archie            & Tortuosity factor in Archie equation &  double   & 2.0  \\
         m\_Archie            & Porosity exponent in Archie equation &  double   & 0.4  \\
         n\_Archie            & Saturation exponent in Archie equation &  double   & 1.13  \\
...
In WAVE-Simulation, we have introduced that \verb+useRandSource+ can be used to apply random shot inversion. Here, we show an example of random shot inversion with \verb+useRandSource+ = 1, \verb+NumShotDomains+ = 3 and 6 shots in total:
\begin{verbatim}
...
# Stage | Iteration | shotNumbers
    ...
    1         2        3   4   5
\end{verbatim}
If \verb+misfitType=L278+, then file \verb+misfitTypeFilename+ will be called to output the misfit type records during the random misfitType inversion.
\begin{verbatim}
...
# Stage | Iteration | misfitTypes
    ...
    1         2       l7  l2  l8
\end{verbatim}
Of course \verb+misfitType+ and \verb+useRandSource+ are independent. So you can implement the random objective waveform inversion (ROWI) \citep{pan2020random} by setting \verb+misfitType=L278+, \verb+useRandSource+ = 1 and \verb+NumShotDomains+ = 1, or implement the ROWI with shot parallelization by setting \verb+misfitType=L278+, \verb+useRandSource+ = 1 and \verb+NumShotDomains+ > 1, or set \verb+useRandSource+ = 1 to implement random source inversion with the same misfit function, or set \verb+misfitType=L278+ to implement random misfit inversion with all shots (\verb+useRandSource+ = 0) or sequential shots (\verb+useRandSource+ = 2).
...
The parameter \verb+DHInversion+ defines the factor of \verb+DH+ for the cross-correlation in the gradient calculation. If e.g., it is set to 2, every second model space sample is picked. The highest possible value depends on the model resolution you want to obtain.
...
\begin{adjustbox}{width=1\textwidth,center=\textwidth}
	...
	\midrule
         ...
         useSeismogramTaper       & Use seismogram taper (0, 1 and 2)               &  int   & 0 (=no) \\
